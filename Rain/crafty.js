...
Crafty.fn = Crafty.prototype = {
    ...
    */
     ...
     * 
     * @triggers Freeze - While the entity is being frozen
     * 
     ...
     * 
     * Because the entity no longer listens to events, modifying its properties can result in an inconsistent state.
     * 
...
Crafty._callbackMethods = {
    ...
    _runCallbacks: function(event, data) {
        if (!this._callbacks[event] || this._callbacks.__frozen) {
            return;
        }
        var callbacks = this._callbacks[event];
    ...
    _unbindAll: function() {
        ...
        delete this._callbacks.__frozen;
    ...
    _freezeCallbacks: function() {
        if (!this._callbacks) return;
        // This sort of shows up in the list of callbacks, so need to delete it on unfreeze or unbindall
        for (var event in this._callbacks) {
            if (this._callbacks[event]) {
...
Crafty._registerLayerTemplate("DOM", {
    ...
    events: {
        "Freeze":function(){
            this._lastParentNode = this._div.parentNode;
            this._div.parentNode.removeChild(this._div);
        },
        "Unfreeze":function(){
            this._lastParentNode.appendChild(this._div);
            delete this._lastParentNode;
        }
    },
...
Crafty.c("Renderable", {
    ...
    events: {
        "Freeze":function(){
            ...
            this.trigger("Invalidate");
            //if (this._drawLayer) this._drawLayer.detach(this);
        },
        "Unfreeze":function(){
            this._visible = !this._hideOnUnfreeze;
            //if (this._drawLayer) this._drawLayer.attach(this);
            this.trigger("Invalidate");
        }
    },
