...
Crafty.c("DynamicCollision", {
    ...
    doCollision: (A, B) ->
        ...
        # If the collision is in the air, we allow more dynamic collisions (FALSE, but was true)
...
Crafty.c("Movable", {
    ...
    glue: (e, x=null, y=null)->
        ...
        return false if e.has("Movable") is false 
        return @glued if @glued.indexOf(e)>=0
    ...
    carry: (e, x=null, y=null)->
        #Giving x&y is a shorthand for also moving the object to a particular offset relative to the parent
        return false if e.has("Movable") is false 
        return @carried if @carried.indexOf(e) >= 0
    ...
    setFree: ()->
        if this.glue_parent?
            this.glue_parent.unglue(this)
        else if this.carry_parent?
            this.carry_parent.detach(this)
...
Crafty.c("Supportable", {
    ...
    _checkSupport: ()->
        ...
        if @feet.hit('Platform')  
            @grounded = true
            groundBlock = Crafty(@feet.hit('Platform')[0].obj[0])
            #console.log(@feet.hit('Platform'))
            #console.log(groundBlock.__c)
            #groundBlock.color("blue")
            #groundBlock.carry(this)
